...
function WorkItemRow({ projectSlug, item }: { projectSlug: string; item: ProjectWorkItem }) {
  const itemHref = `/projects/${encodeURIComponent(projectSlug)}/work-items/${encodeURIComponent(item.itemId)}`;
      ...
      <div className="min-w-0">
        ...
        >
          <Typography
            ...
            tone="primary"
            weight="semibold"
            className="truncate hover:text-prism-navy"
          >
            {item.title}
          </Typography>
        </Link>
